...
def normalize_topic(topic: str) -> str:
    """Нормализация тематик для стабильной группировки"""
...
def load_all_excels() -> pd.DataFrame:
    ...
    model = get_model()
    df.attrs["phrase_embs"] = model.encode(
        df["phrase_proc"].tolist(),
...
# ---------- поиск ----------
def semantic_search(query: str, df: pd.DataFrame, top_k=5, threshold=0.5):
    model = get_model()
...
def keyword_search(query: str, df: pd.DataFrame):
    query_proc = preprocess(query)
...
def filter_by_topics(results, selected_topics):
    ...
    selected_topics = {normalize_topic(t) for t in selected_topics}
    filtered = []

    for item in results:
        if len(item) == 4:
            score, phrase, topics, comment = item
            if set(topics) & selected_topics:
